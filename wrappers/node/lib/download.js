...
const REPO = 'CloudCannon/inane';
const userAgent = 'inane';
...
async function unzipExec(zipPath, destinationDir) {
    const expectedName = path.join(destinationDir, execname);
    ...
    throw new Error(`Expecting ${execname} or ${execname}.exe unzipped into ${destinationDir}, didn't find one.`);
}
...
module.exports = async opts => {
    ...
    const assetName = [execname, opts.version, opts.target].join('-') + '.tar.gz';
    ...
    try {
        ...
        try {
            ...
            const expectedName = path.join(opts.destDir, execname);
    ...
    try {
        const destinationPath = await unzipExec(assetDownloadPath, opts.destDir);
        if (!isWindows) {
